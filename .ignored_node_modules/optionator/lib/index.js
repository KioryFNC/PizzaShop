...
;(function () {
  ...
  main = function (libOptions) {
    ...
    libOptions.positionalAnywhere == null &&
      (libOptions.positionalAnywhere = true)
      ...
      for (i$ = 0, len$ = options.length; i$ < len$; ++i$) {
        ...
        if (option.heading == null) {
          ...
          if (option['enum'] && !option.parsedPossiblities) {
            ...
            for (
              j$ = 0, len1$ = (ref$ = option['enum']).length;
              j$ < len1$;
              ++j$
            ) {
              possibility = ref$[j$]
          ...
          if ((that = option.dependsOn)) {
            if (that.length) {
              ...
              if (dependsOpts.length) {
                ...
                if (
                  (ref$ = dependsType.toLowerCase()) === 'and' ||
                  ref$ === 'or'
                ) {
          ...
          if (option.alias || option.aliases) {
            ...
            if (option.alias) {
              option.aliases == null &&
                (option.aliases = [].concat(option.alias))
            }
            for (
              j$ = 0, len1$ = (ref$ = option.aliases).length;
              j$ < len1$;
              ++j$
            ) {
              alias = ref$[j$]
        ...
        for (i$ = 0, len$ = targetOptionNames.length; i$ < len$; ++i$) {
          ...
          if (targetOption && checkDependency(targetOption)) {
            ...
            throw new Error(
              "The option '" +
                option.option +
                "' did not have its dependencies met.",
            )
          }
        }
        if (type === 'and') {
          ...
          throw new Error(
            "The option '" +
              option.option +
              "' did not meet any of its dependencies.",
          )
        }
      }
