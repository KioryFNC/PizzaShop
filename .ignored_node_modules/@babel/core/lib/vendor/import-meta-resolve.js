  ...
  (packagePath, key, target, isImport = false, base = undefined) => {
    ...
    if (key === '.') {
      ...
      return (
        ...
        `in the package config ${packagePath}package.json${base ? ` imported from ${base}` : ''}${relatedError ? '; targets must start with "./"' : ''}`
      )
    }
...
function read(jsonPath, { base, specifier }) {
  ...
  let string
  try {
    string = _fs().default.readFileSync(
      _path().toNamespacedPath(jsonPath),
      'utf8',
    )
  } catch (error) {
  ...
  if (string !== undefined) {
    ...
    try {
      ...
      const error = new ERR_INVALID_PACKAGE_CONFIG$1(
        jsonPath,
        (base ? `"${specifier}" from ` : '') +
          (0, _url().fileURLToPath)(base || specifier),
        cause.message,
      )
      error.cause = cause
      throw error
...
function emitLegacyIndexDeprecation(url, packageJsonUrl, base, main) {
  ...
  const packagePath = (0, _url().fileURLToPath)(
    new (_url().URL)('.', packageJsonUrl),
  )
  ...
  if (!main) {
    ...
    _process().emitWarning(
      `Package ${packagePath} has a "main" field set to "${main}", ` +
        `excluding the full filename and extension to the resolved file at "${urlPath.slice(packagePath.length)}", imported from ${basePath}.\n Automatic extension resolution of the "main" field is ` +
...
function finalizeResolution(resolved, base, preserveSymlinks) {
  ...
  if (!preserveSymlinks) {
    ...
    resolved = (0, _url().pathToFileURL)(
      real + (filePath.endsWith(_path().sep) ? '/' : ''),
    )
...
function packageResolve(specifier, base, conditions) {
  ...
  let packageJsonUrl = new (_url().URL)(
