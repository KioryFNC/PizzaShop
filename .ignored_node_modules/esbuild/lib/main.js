  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== '0.25.2') {
        throw new Error(
          `Cannot start service: Host version "${'0.25.2'}" does not match binary version ${quote(binaryVersion)}`,
        )
      }
      return
    }
...
var ensureServiceIsRunning = () => {
  ...
  let child = child_process.spawn(
    command,
    args.concat(`--service=${'0.25.2'}`, '--ping'),
    {
      windowsHide: true,
